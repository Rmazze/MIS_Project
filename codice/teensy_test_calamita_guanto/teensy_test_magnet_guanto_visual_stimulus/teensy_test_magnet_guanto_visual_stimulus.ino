...
bool hand_sens_reading;
bool hand_sens_state;             // the current reading from the input pin
bool last_hand_sens_state = LOW;   // the previous reading from the input pin

unsigned long last_debounce_time_hand = 0;  // the last time the output pin was toggled
...
void setup() {
  ...
  pinMode(hand_sens, INPUT_PULLUP);
  ...
  attachInterrupt(digitalPinToInterrupt(hand_sens), stop_test_ISR, RISING);
}
