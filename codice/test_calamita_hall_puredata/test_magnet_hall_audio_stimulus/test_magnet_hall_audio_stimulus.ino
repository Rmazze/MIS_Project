...
#define audio_stimulus_output 7
...
void setup() {
  ...
  pinMode(audio_stimulus_output, OUTPUT);
  ...
  digitalWrite(audio_stimulus_output, audio_stimulus_state);
...
void loop() {
  switch(program_execution_state){
    ...
    case(1):  if ((millis() - test_time_ready) >= rand_time){
                audio_stimulus_state = HIGH;
                magnet_state = LOW;
                program_execution_state = 2;
                test_time_start = millis();
                hall_led_start_state = HIGH;

                Serial.print("audio_stimulus_output, ");
                Serial.println(audio_stimulus_state);    
              }
